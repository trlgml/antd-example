class Palette {
  constructor(canvas, { drawType = 'line', drawColor = '#FF4500', lineWidth = 5, sides = 3, allowCallback, moveCallback }) {
    this.canvas = canvas;
  ...
  snapshot(link, name) {
    let image = new Image();
    image.src = this.canvas.toDataURL({ format: 'image/png', quality: 1, width: 14000, height: 4000 });
  ...
  onmousemove(e) { // 鼠标移动
    ...
    switch (this.drawType) {
      ...
      case 'eraser': {
        ...
        break;
      case 'ellipse': {
        let params = [this.x, this.y, Math.abs(width), Math.abs(height), this.lineWidth, this.drawColor];
        this.moveCallback('ellipse', ...params);
        this.ellipse(...params);
      }
        break;
      default:
  ...
  ellipse(x, y, width, height, lineWidth, drawColor) {
    this.reSetImage();
    this.paint.beginPath();
    this.paint.lineWidth = lineWidth;
    this.paint.ellipse(x, y, width, height, 0 * Math.PI / 180, 0, 2 * Math.PI);
    this.paint.strokeStyle = drawColor;
    this.paint.closePath();
    this.paint.stroke();
  }
